...
export class ListComponent extends BaseComponent implements OnInit {
  ...
  displayedColumns: string[] = ['photo', 'name', 'stock', 'price', 'createdDate', 'updatedDate', 'showcase', 'qrcode', 'edit', 'delete'];
  ...
  async getProducts() {
    ...
    const allProducts: { totalProductCount: number; products: List_Product[] } = await this.productService.read(this.paginator ? this.paginator.pageIndex : 0, this.paginator ? this.paginator.pageSize : 5, this.activeCategoryId || undefined, undefined, this.searchTerm, undefined, () => this.hideSpinner(SpinnerType.BallAtom), errorMessage => this.alertifyService.message(errorMessage, {
      dismissOthers: true,
